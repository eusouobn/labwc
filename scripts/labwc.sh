...
OFFICIAL_PACKAGES=(
  ...
  opencode github-cli lightdm lightdm-gtk-greeter
)
...
# ──────────────────────────────────────────────
# Dolphin: servicemenu para montar/desmontar ISO
# ──────────────────────────────────────────────
step "💿 Servicemenu de ISO no Dolphin"
mkdir -p "$HOME/.config/scripts" "$HOME/.local/share/kio/servicemenus"
tee "$HOME/.config/scripts/iso-mount.sh" > /dev/null <<'EOF'
#!/usr/bin/env bash
...
EOF
chmod +x "$HOME/.config/scripts/iso-mount.sh"

tee "$HOME/.local/share/kio/servicemenus/mount-iso.desktop" > /dev/null <<EOF
[Desktop Entry]
Type=Service
ServiceTypes=KonqPopupMenu/Plugin
MimeType=application/x-cd-image;application/x-iso9660-image;application/vnd.efi.img;application/x-raw-disk-image;application/octet-stream;
Actions=mount;unmount;
X-KDE-Priority=TopLevel

[Desktop Action mount]
Name=Montar imagem de disco
Icon=media-optical
Exec=$HOME/.config/scripts/iso-mount.sh %f mount

[Desktop Action unmount]
Name=Desmontar imagem de disco
Icon=media-optical
Exec=$HOME/.config/scripts/iso-mount.sh %f unmount
EOF

kbuildsycoca6 >/dev/null 2>&1 || true
ok "Servicemenu de ISO instalado (clique direito em .iso)"
